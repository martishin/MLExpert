class TreeNode:
    ...
    def split(self):
        ...
        for feature in self.examples[0].keys():
            ...
            for i, _ in enumerate(self.examples[:-1]):
                split_point_value = (
                    self.examples[i][feature] + self.examples[i + 1][feature]
                ) / 2
    ...
    def get_split_point_mse(self, feature, split_point_value):
        left_split_labels = [
            ...
            if example[feature] <= split_point_value
        ]
        right_split_labels = [
            example["bpd"]
            for example in self.examples
        ...
        mse = (
            (len(left_split_labels) * left_split_mse)
            + (len(right_split_labels) * right_split_mse)
        ) / num_samples
